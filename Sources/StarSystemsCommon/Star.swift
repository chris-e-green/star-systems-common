...
class Star: Satellite, /*Hashable, */ Equatable, CustomStringConvertible {
    static func == (lhs: Star, rhs: Star) -> Bool {
        return lhs.starDetail == rhs.starDetail && lhs.magnitude == rhs.magnitude
    }
    ...
    convenience init(type: StarType, decimal: Int, size: StarSize, parent: Star? = nil, verbose: Bool = false) {
        self.init(starType: type, starDecimal: decimal, starSize: size, verbose: verbose)
    ...
    init(starType: StarType = .A, starDecimal: Int = 0, starSize: StarSize = .Ia, parentStar: Star? = nil, verbose: Bool = false) {
    ...
    convenience init(companion: Bool, typeRoll: Int, sizeRoll: Int, parent: Star? = nil, verbose: Bool = false) {
    ...
    func getAvailOrbit() -> Float {
        ...
        if verbose { print("Avail on \(name); AvailOrbits returned \(possibilities)") }
    ...
    func getAnyAvailOrbit() -> Float {
        ...
        if verbose { print("Avail on \(name); AnyAvailOrbits returned \(possibilities)") }
        orbit = possibilities[Dice.roll(1, sides: possibilities.count)-1]
        return orbit
    }
    ...
    func getAvailInOutOrbit() -> Float? {
        ...
        if verbose { print("AvailInOut on \(name); AvailOrbits returned \(possibilities)") }
    ...
    func getAvailOrbits(_ zones: Set<Zone>, createIfNone: Bool=true) -> [Float] {
        ...
        if createIfNone {
            ...
            if orbits.count == 0 {
                ...
                if verbose { print("Created a new orbit number \(maxOrbitNum) on \(name)") }
 ...
 */
    func getAvailHabitableOrOuterOrbit() -> Float? {
        ...
        if verbose { print("AvailHabOut on \(name); AvailOrbits returned \(possibilities)") }
        if possibilities.count > 0 {
            orbit = possibilities[Dice.roll(1, sides: possibilities.count)-1]
        }
        return orbit
