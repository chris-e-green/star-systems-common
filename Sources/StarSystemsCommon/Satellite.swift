        ...
        do {
            ...
            if (getSatellite(orbit) is EmptyOrbit ||
                ...
                newSatellite is EmptyOrbit {
                    //os_log("Tried to add an empty orbit and collided with another companion", log: OSLog.default, type: .info)
            } else {
    ...
    func getPlanets() -> [Planet] {
        ...
        for (_, satellite) in satellites.orbits {
            if let planet = satellite as? Planet {
                ...
                //os_log("\(satellite.name) is not a planet", log: OSLog.default, type: .info)
            }
