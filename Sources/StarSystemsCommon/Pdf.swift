...
class Pdf {
    ...
    #endif
    var currObjId = 2
    var addPdfComments: Bool

    init(addPdfComments: Bool = false) {
        self.addPdfComments = addPdfComments
    }
    ...
    func circle(_ x: Double, y: Double, radius: Double, filled: Bool, blue: Bool) -> String {
        ...
        var circleStr = ""
        if addPdfComments { circleStr += "% circle at (\(x~), \(y~)) r=\(radius~)\n" }
        if filled && blue {
    ...
    func asteroids(_ x: Double, y: Double, size: Double) -> String {
        var asteroidStr = ""
        if addPdfComments { asteroidStr += "% asteroid at (\(x~), \(y~))\n" }
    ...
    func triangle(_ x: Double, y: Double, size: Double) -> String {
        var triangleStr = ""
        if addPdfComments { triangleStr += "% triangle at (\(x~), \(y~))\n" }
    ...
    func star(_ x: Double, y: Double, size: Double) -> String {
        ...
        var starStr = ""
        if addPdfComments { starStr += "% asterisk at (\(x~), \(y~))\n" }
    ...
    func drawHex(_ x: Double, y: Double, height: Double) -> String {
        ...
        var hexString = ""
        if addPdfComments { hexString += "% hex at (\(x~), \(y~))\n" }
        hexString += """
    ...
    func hex(_ x: Double, y: Double, height: Double, label: String) -> String {
        var hexStr = ""
        if addPdfComments { hexStr += "% hex at (\(x~), \(y~))\n" }
    ...
    func display(_ planet: Planet) {
        ...
        var pdfText: String = ""
        if addPdfComments { pdfText += "% Rendering \(planet.name) to list\n" }
        ...
        if addPdfComments { mapData += "% Rendering \(planet.name) to map\n" }
