...
class StarSystem: CustomStringConvertible {
    ...
    var allPlanets: [Planet] {
        ...
        for star in stars {
            ...
            if verbose { print("added \(allPlanets) to allPlanets") }
        }
        return result
    }
    ...
    var allGGinHabOrbits: [GasGiant] {
        ...
        for (star, orbit) in allHabitable {
            if let gasGiant = star.getSatellite(orbit) as? GasGiant {
                if verbose { print("found a gas giant") }
    ...
    var allNavalBaseCandidates: [Planet] {
        ...
        for planet in allPlanets {
            if planet.internalPopulation >= 3 && planet != mainWorld! {
                ...
                if verbose { print("added \(planet) to naval base candidates") }
            } else {
                if verbose {
                    if planet == mainWorld! {
    ...
    var allScoutBaseCandidates: [Planet] {
        ...
        for planet in allPlanets {
            if planet.internalPopulation >= 2 && planet != mainWorld! {
                result.append(planet)
                if verbose { print("added \(planet) to scout base candidates") }
            } else {
                if verbose {
                    if planet == mainWorld! {
                        print("\(planet) is the mainworld, not added to candidates.")
    ...
    var availGGHabOrbit: GasGiant? {
        ...
        if verbose { print("possible GGs count=\(possCount)") }
    ...
    init(newWorld: Planet? = nil, verbose: Bool = false) {
        ...
        if newWorld != nil {
            ...
            if newWorld!.internalAtmosphere <= 1 || newWorld!.internalAtmosphere >= 10 || newWorld!.internalSize == 0 {
                if verbose { print("mainworld does not need habitable zone") }
        ...
        repeat {
            ...
            for companionIdx in 0..<basicNature.rawValue {
                ...
                if verbose { print("companion \(companion.name) maxOrbitNum started at \(companion.maxOrbitNum)") }
                if farCompanion {
                    var farOrbit: Float
                    repeat {
                    ...
                    if verbose { print("far companion \(farComp.name) maxOrbitNum started at \(farComp.maxOrbitNum)") }
                ...
                if verbose { print("companion added at orbit \(orbit) on primary") }
            }
            ...
            if verbose { print("primary \(primary.name) maxOrbitNum started at \(primary.maxOrbitNum)") }
            ...
            for compOrbit in compOrbits {
                ...
                if compOrbit > 0 { // close orbits don't make sense for this bit.
                    ...
                    for unavailableOrbit in unavailableOrbits..<compOrb {
                        if primary.getSatellite(unavailableOrbit) == nil {
                            ...
                            if verbose { print("companion; emptied orbit \(unavailableOrbit) on primary") }
                        }
                    }
                ...
                if verbose { print("companion; emptied orbit \(Float(compOrb + 1)) on primary") }
            }
            if verbose { if allAvailHabitable.count == 0 && mainWorldSupplied { print("no habitable zones, regenerating!") } }
        ...
        for star in stars {
            ...
            if verbose { print("rolled \(emptyOrbits) empty orbits on \(star.name)") }
            ...
            if emptyOrbits > usableOrbits {
                ...
                if verbose { print("reduced empty orbits to \(emptyOrbits)") }
            }
            // swiftlint:disable todo
            ...
            if star.maxOrbitNum >= 2 {
                for emptyOrbit in 0 ..< emptyOrbits {
                    ...
                    if verbose { print("emptied orbit \(orbit) on star \(star.name)") }
                }
            } else {
                if verbose { print("max orbit number less than two, so impossible to roll one on 2D") }
            }
            ...
            if verbose { print("rolled \(capturedPlanets) captured planets on \(star.name)") }
            ...
            for capturedPlanet in 0 ..< capturedPlanets {
                ...
                if verbose { print("captured planet in orbit \(orbit) on star \(star.name)") }
            }
        }
        ...
        for star in stars {
            ...
            for habitableInnerOuterOrbit in habitableInnerOuterOrbits {
                ...
                if verbose { print("added \(star.name), orbit \(habitableInnerOuterOrbit) to all list") }
            }
            for habitableOrbit in habitableOrbits {
                allHabitable.append((star, habitableOrbit))
                if verbose { print("added \(star.name), orbit \(habitableOrbit) to habitable list") }
            }
            for habitableOuterOrbit in habitableOuterOrbits {
                allHabOuter.append((star, habitableOuterOrbit))
                if verbose { print("added \(star.name), orbit \(habitableOuterOrbit) to hab/outer list") }
            }
        }
        if verbose {
        ...
        if verbose { print("gg: \(avail) available habitable and outer orbits") }
        // swiftlint:disable todo
        ...
        if gasGiants {
            ...
            if verbose { print("rolled \(gasGiantCount) gas giants") }
            
            if gasGiantCount > avail {
                gasGiantCount = avail
                if verbose { print("reduced gas giant count to \(gasGiantCount)") }
            }
            avail -= gasGiantCount
        }
        ...
        if planetoids {
            ...
            if verbose { print("rolled \(planetoidBelts) planetoid belts") }
        ...
        if gasGiants && gasGiantCount == 0 {
            ...
            if verbose { print("added only gas giant \(gasGiant.name) to new orbit \(orbit) on \(ggStar.name)") }
        } else {
            for _ in 0..<gasGiantCount {
                if let (star, orbit) = availHabOuterOrbit {
                    ...
                    if verbose { print("added gas giant \(gasGiant.name) to orbit \(orbit) on \(star.name)") }
                } else {
                    if verbose { print("got no place for gg") }
                }
        ...
        for _ in 0..<planetoidBelts {
            ...
            if let (star, orbit) = !mainWorldSupplied ? availOrbit : availInOutOrbit {
                ...
                if verbose { print("added planetoid to \(orbit) on \(star.name)") }
            } else {
                if verbose { print("Got no place for planetoid") }
            }
            //            }
        }
        ...
        if mainWorldSupplied && !mainWorldPlaced {
            if let (star, mwOrbit) = mainWorldNeedsHabitable ? availHabOrbit : availOrbit {
                ...
                if verbose { print("placed main world as satellite of \(star.name) in orbit \(mwOrbit)") }
            } else {
                if let gasGiant = availGGHabOrbit {
                    ...
                    mainWorldPlaced = true // don't place the mainworld on each star...
                    if verbose { print("placed main world as satellite of gas giant \(gasGiant.name)") }
                    
                } else {
                    if verbose { print("didn't find an available habitable orbit") }
                }
            }
        }
        ...
        for star in stars {
            ...
            for orbit in star.getAvailOrbits(Set<Zone>([Zone.H, Zone.I, Zone.O])) {
                ...
                star.addToOrbit(orbit, newSatellite: planet)
                if verbose { print("Added planet to orbit \(orbit) on star \(star.name)") }
            }
        ...
        if let mainWorldPlanet = mainWorld {
            ...
            if verbose { print("Primary contains \(primary.getPlanets().count) planets.") }
